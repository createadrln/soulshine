...
if (isset($_REQUEST['submit']) || isset($_REQUEST['libsyn_settings_submit'])) { //has showSelect on form.
	...
	if($api instanceof \Libsyn\Api) { //Brand new setup or changes?
		if(!empty($_REQUEST['submit']) && ($_REQUEST['submit']==='Save Player Settings' || $_REQUEST['submit']==='Save+Player+Settings')) { //has Player Settings Update
			...
			if(!is_null($api->getClientId())) { //check for cleared data
				...
				if(!isset($_REQUEST['feed_redirect_url'])) $_REQUEST['feed_redirect_url'] = '';
				if($api->getFeedRedirectUrl()!==$_REQUEST['feed_redirect_url']) $api->setFeedRedirectUrl($_REQUEST['feed_redirect_url']);
				$update = $plugin->updateSettings($api);
				if($update!==false) $msg = __('Settings Updated',$libsyn_text_dom);
				
				//do feed import
				$show_id = $api->getShowId();
				if($api->getFeedRedirectUrl() !== $_REQUEST['feed_redirect_url'] && !empty($_REQUEST['feed_redirect_url']) && !empty($show_id)) {
				...
				if(!empty($_REQUEST['clientId']) && !empty($_REQUEST['clientSecret'])) { 
					update_option('libsyn-podcasting-client', array('id' => $sanitize->clientId($_REQUEST['clientId']), 'secret' => $sanitize->clientSecret($_REQUEST['clientSecret']))); 
...
if((!$api) || ($api instanceof \Libsyn\Api && $api->isRefreshExpired())) { //does not have $api setup yet in WP
	...
	if(!$authorized) {
		...
		if(!empty($libsyn_code)) {
			...
			if(!empty($clientId)) {
				$clientId = $clientId;
			} elseif($client = get_option('libsyn-podcasting-client') && !empty($client['id'])) {
				$clientId = $client['id'];
			} else {
			...
			} elseif($client = get_option('libsyn-podcasting-client') && !empty($client['secret'])) {
...
<?php wp_enqueue_style( 'colorPickerStyle', plugins_url(LIBSYN_DIR.'/lib/css/jquery.colorpicker.css' )); ?>
	...
	<?php if (isset($msg)) echo $plugin->createNotification($msg, $error); ?>
	  <h2><?php _e("Libsyn Plugin Settings", $libsyn_text_dom); ?><span style="float:right"><a href="http://www.libsyn.com/"><img src="<?php echo plugins_url( LIBSYN_DIR . '/lib/images/libsyn_dark-small.png'); ?>" title="Libsyn Podcasting" height="28px"></a></span></h2>
		  ...
		  <div id="post-body">
			...
			<!-- BOS Existing API -->
			  <div class="stuffbox" style="width:93.5%">
				...
				<div class="inside" style="margin: 15px;">
				  <p><em><?php _e("Libsyn account application settings can be setup and viewed by logging into your <a href=\"https://www.libsyn.com/\" target=\"_blank\">Libsyn Account.</a> then click the Arrow in the top right and clicking \"Manage Wordpress Plugins\"", $libsyn_text_dom); ?></em></p>
			...
			<!-- BOS Add new API -->
			  <div class="stuffbox">
				...
				<div class="inside" style="margin: 15px;">
				  <p><em><?php _e("Enter settings provided by logging into your <a href=\"https://youtu.be/mMPSkd7YlRc\" target=\"_blank\"><strong><em>Libsyn account</em></strong></a> then selecting the arrow in the top right and clicking \"Manage Wordpress Plugins\"", $libsyn_text_dom); ?></em></p>
				  <table class="form-table">
					<tr valign="top">
					  <th><?php _e("Client ID:", $libsyn_text_dom); ?></th>
					  <td>
							...
							<tr valign="top">
								...
								<td>
									<div>
										...
										<br />
										<div>
											<input id="player_use_theme_custom" type="radio" value="custom" name="player_use_theme" /><span style="margin-left:16px;"><strong>Custom</strong>&nbsp;&nbsp;<em style="font-weight:300;">(minimum height 90px or 300px(video), width 100% fixed)</em></span>
										</div>
		...
		(function($){
			$(document).ready(function(){
				...
				$(".box_left_content").fadeIn('fast', function() {
						...
						$("#player_use_theme_standard_image").append('<img src="<?php echo plugins_url( LIBSYN_DIR . '/lib/images/player-preview-standard.jpg'); ?>" />');
						$("#player_use_theme_mini_image").append('<img src="<?php echo plugins_url( LIBSYN_DIR . '/lib/images/player-preview-standard-mini.jpg'); ?>" />');
						$("#player_use_theme_custom_image").append('<img src="<?php echo plugins_url( LIBSYN_DIR . '/lib/images/custom-player-preview.jpg'); ?>" />');
					...
					var playerUseThumbnail = '<?php echo $playerUseThumbnail; ?>';
					...
					var playerUseDownloadLinkText = '<?php echo $playerUseDownloadLinkText; ?>';
						...
						if(typeof libsyn_player_color_picker == 'undefined') {
							...
							var libsyn_player_color_picker = $('#player_custom_color').colorpicker({
								...
								, open: function(ui, event) {
									 $(".ui-dialog-titlebar-close", ui.dialog | ui).css({"position": "absolute", "right": 10, "top": 4}).attr("class", "ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only ui-dialog-titlebar-close");
								}
