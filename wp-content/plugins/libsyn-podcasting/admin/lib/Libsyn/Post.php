...
class Post extends \Libsyn{
	...
	public static function actionsAndFilters() {
		...
		add_action('wp_ajax_fetch_custom_list', array('\Libsyn\Post', 'fetch_custom_list_callback')); // (handles ajax destinations calls)
	}
						...
						buttons: [
							...
							},
							{
								...
								click: function(){
									...
									mediaUploadForm.append('show_id', '<?php if(isset($api)) echo $api->getShowId(); ?>');
									mediaUploadForm.append('form_access_token', '<?php if(isset($api)) echo $api->getAccessToken(); ?>');
			...
			<table class="form-table">
					  ...
					  <td>
						<div id="libsyn-primary-media-settings">
							<div id="libsyn-new-media-settings">
								...
								<label for="libsyn-new-media-media"><?php echo __( 'Media Url', 'libsyn-nmp' ); ?></label> <input type="url" id="libsyn-new-media-media" name="libsyn-new-media-media" value="<?php echo esc_attr( $libsyn_media_media ); ?>" pattern="https?://.+" <?php if(isset($libsyn_media_media)&&!empty($libsyn_media_media)) echo 'readonly'; ?>/>
				...
				<tr valign="top">
					  ...
					  <td>
						<div id="titlediv">
							<div id="titlewrap">
								<input id="libsyn-post-episode-subtitle" type="text" autocomplete="off" value="<?php echo get_post_meta( $object->ID, 'libsyn-post-episode-subtitle', true ); ?>" name="libsyn-post-episode-subtitle" style="width:100%;" maxlength="255">
							</div>
						</div>
					  </td>
				</tr>
				...
				<tr valign="top">
					  ...
					  <td>
						<div id="titlediv">
							<div id="titlewrap">
								<input id="libsyn-post-episode-keywords" type="text" autocomplete="off" value="<?php echo get_post_meta( $object->ID, 'libsyn-post-episode-keywords', true ); ?>" name="libsyn-post-episode-keywords" style="width:100%;" maxlength="255" placeholder="keyword1, keyword2, keyword3">
							</div>
						</div>
					  </td>
				</tr>
	 ...
	 */
	function fetch_custom_list_callback() {
		$destination = new Service\Destination();
		...
		if(!$isUpdatePost) {
			if($playerPlacement==='top') {
				$wpdb->update(
					$wpdb->prefix . 'posts',
					array(
						'post_content' => '[podcast src="'.$libsyn_post->url.'" height="'.$playerHeight.'" width="'.$playerWidth.'" placement="'.$playerPlacement.'" theme="'.$playerTheme.'"]'.$download_link.wp_kses_post(self::stripShortcode('podcast', $post->post_content)),
				...
				$wpdb->update(
					$wpdb->prefix . 'posts',
					array(
						'post_content' => wp_kses_post(self::stripShortcode('podcast', $post->post_content)).'[podcast src="'.$libsyn_post->url.'" height="'.$playerHeight.'" width="'.$playerWidth.'" placement="'.$playerPlacement.'" theme="'.$playerTheme.'"]'.$download_link,
						'post_modified' => date("Y-m-d H:i:s"),
						'post_modified_gmt' => gmdate("Y-m-d H:i:s"),
					),
					array('ID' => $post->ID)
				...
				if (isset($matches[2]) && $matches[2] == 'podcast') { // matches (has player shortcode)
					...
					if($playerPlacement==='top') {
						$wpdb->update(
							$wpdb->prefix . 'posts',
							array(
								'post_content' => '[podcast src="'.$libsyn_post->url.'" height="'.$playerHeight.'" width="'.$playerWidth.'" placement="'.$playerPlacement.'" theme="'.$playerTheme.'"]'.$download_link.wp_kses_post(self::stripShortcode('podcast', $new_post_content)),
								'post_modified' => date("Y-m-d H:i:s"),
								'post_modified_gmt' => gmdate("Y-m-d H:i:s"),
							),
							array('ID' => $post->ID)
						...
						$wpdb->update(
							$wpdb->prefix . 'posts',
							array(
								'post_content' => wp_kses_post(self::stripShortcode('podcast', $new_post_content)).'[podcast src="'.$libsyn_post->url.'" height="'.$playerHeight.'" width="'.$playerWidth.'" placement="'.$playerPlacement.'" theme="'.$playerTheme.'"]'.$download_link,
								'post_modified' => date("Y-m-d H:i:s"),
								'post_modified_gmt' => gmdate("Y-m-d H:i:s"),
							),
							array('ID' => $post->ID)
