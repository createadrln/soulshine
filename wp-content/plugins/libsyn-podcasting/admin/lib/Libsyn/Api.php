...
class Api extends \Libsyn{
	...
	public function refreshToken() {
		...
		*/
		...
		if(!$checkResponse) { //bad check will remove refresh token and return false so user can re-authenticate oauth
			...
			$this->save();
			if($this->hasLogger) $this->logger->error("API:\trefreshToken:\tcheckResponse is false.");
			return false;
		}
		...
		if(!empty($data->access_token) && !empty($data->refresh_token)) {
			...
			if($this->hasLogger) $this->logger->error("API:\trefreshToken:\taccess_token or refresh_token is not set.");
			return false;
		}
	}
	...
	// public function retrieveApiById( $plugin_api_id ) {
		...
		// } else {
			// if($this->hasLogger) $this->logger->error("API:\trefreshToken:\tCould not retrieve API data set.");
			// return false;
		// }
	...
	public function isRefreshExpired() {
		...
		if(!empty($checkDate)) {
			if ($this->wp_time >=  strtotime($checkDate)) {
				...
				if($this->hasLogger) $this->logger->info("API:\tisRefreshExpired:\t".date("Y-m-d H:i:s", $this->wp_time)." not less than ".$this->refresh_token_expires);
			}
		} else {
			if($this->hasLogger) $this->logger->error("API:\tisRefreshExpired:Refresh Token Expires Empty");
		}
	...
	public function update( array $settings ) {
		...
		if(isset($settings['access_token']) && isset($settings['refresh_token'])) {
			...
			if($this->hasLogger) $this->logger->error("API:\tupdate:\taccess_token or refresh_token not set.");
			if($this->hasLogger) $this->logger->error("API:\tupdate:\taccess_token:\t".$settings['access_token']);
			if($this->hasLogger) $this->logger->error("API:\tupdate:\trefresh_token:\t".$settings['refresh_token']);
			return false;
		}
	}
		...
		if($obj->response['code']!==200) {
			if(isset($this->logger)) {
				if($this->hasLogger) { // log errors
					$this->logger->error("API:\tcheckResponse:\tError");
					if(!empty($obj->response['url']))
						$this->logger->error("API:\turl:\t".$obj->response['url']);
							else $this->logger->error("API:\turl:\tunkown");
					if(!empty($obj->response['success']))
						$this->logger->error("API:\tsuccess:\t".$obj->response['success']);
							elseif(!empty($obj->response['message']))
								$this->logger->error("API:\tmessage:\t".$obj->response['message']);
									else $this->logger->error("API:\tsuccess:\tunkown");
					if(!empty($obj->response['status_code']))
						$this->logger->error("API:\tstatus_code:\t".$obj->response['status_code']);
							elseif(!empty($obj->response['code']))
								$this->logger->error("API:\tstatus_code:\t".$obj->response['code']);
									else $this->logger->error("API:\tstatus_code:\tunkown");
					if(!empty($obj->http_response)){
						...
						if(!empty($objectResponse)) {
							...
							if(!empty($res_data->title)) $this->logger->error("API:\ttitle:\t".$res_data->title);
							if(!empty($res_data->detail)) $this->logger->error("API:\tdetail:\t".$res_data->detail);
						}
					}
				}
			}
